...
from app.services.telegram_contacts import get_telegram_contact
...
async def create_order(
    ...
    customer_phone = normalize_phone_number(
        payload.customerPhone or get_telegram_contact(user_chat_id) or ""
    )
    ...
    stored_first_name = (
        first_name
        if customer_phone in first_name
        else f"{first_name} ({customer_phone})"
    )
